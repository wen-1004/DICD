...
// RTL wrapper
#include "rtl_wrap/minus_rtl_wrap.hpp"
#include "rtl_wrap/argmax_rtl_wrap.hpp"
#include "rtl_wrap/angle_rtl_wrap.hpp"
#include "rtl_wrap/delay_n_rtl_wrap.hpp"
#include "rtl_wrap/phi_sum_rtl_wrap.hpp"
#include "rtl_wrap/gamma_sum_rtl_wrap.hpp"
#include "rtl_wrap/mag_rtl_wrap.hpp"
...
struct EstimatorTop : sc_core::sc_module {
    ...
    sc_core::sc_in<in_r_t>   r_in_real         {"r_in_real"};
    sc_core::sc_in<in_r_t>   r_in_imag         {"r_in_imag"};
    ...
    // ---------- Instances ----------
#ifdef USE_RTL_DELAY_N
    delay_n_rtl_wrap *U_DELAY_N   {nullptr};
#else
    delay_n     *U_DELAY_N   {nullptr};
#endif
#ifdef USE_RTL_PHI_SUM
    phi_sum_rtl_wrap *U_PHI_SUM   {nullptr};
#else
    phi_sum     *U_PHI_SUM   {nullptr};
#endif
#ifdef USE_RTL_GAMMA_SUM
    gamma_sum_rtl_wrap *U_GAMMA_SUM {nullptr};
#else
    gamma_sum   *U_GAMMA_SUM {nullptr};
#endif
#ifdef USE_RTL_MAG
    mag_rtl_wrap *U_MAG       {nullptr};
#else
    mag         *U_MAG       {nullptr};
#endif
#ifdef USE_RTL_ANGLE
    angle_rtl_wrap *U_ANGLE     {nullptr};
#else
...
#endif
#ifdef USE_RTL_ARGMAX
    argmax_rtl_wrap *U_ARGMAX {nullptr};
#else
...
#ifdef USE_RTL_DELAY_N
        U_DELAY_N   = new delay_n_rtl_wrap("delay_n_rtl");
#else
        U_DELAY_N   = new delay_n   ("delay_n");
#endif
#ifdef USE_RTL_PHI_SUM
        U_PHI_SUM   = new phi_sum_rtl_wrap("phi_sum_rtl");
#else
        U_PHI_SUM   = new phi_sum   ("phi_sum");
#endif
#ifdef USE_RTL_GAMMA_SUM
        U_GAMMA_SUM = new gamma_sum_rtl_wrap("gamma_sum_rtl");
#else
        U_GAMMA_SUM = new gamma_sum ("gamma_sum");
#endif
#ifdef USE_RTL_MAG
        U_MAG       = new mag_rtl_wrap("mag_rtl");
#else
        U_MAG       = new mag       ("mag");
#endif
#ifdef USE_RTL_ANGLE
    U_ANGLE     = new angle_rtl_wrap("angle_rtl");
#else 
...
#endif
#ifdef USE_RTL_ARGMAX
    U_ARGMAX    = new argmax_rtl_wrap("argmax_rtl");
#else
