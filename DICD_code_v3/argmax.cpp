  ...
  void proc() {
    ...
    while (true) {
      if (!rst.read()) {
        ...
        // (2) 尋找最大值索引（0=最新, 255=最舊）
        int pos = 0;
        fx_lambda_t maxv = buf[0];
        // 扫描时同值用 i>pos 保留“更旧”的位置（尾端），避免在边界抖动
        for (int i = 1; i < N; ++i) {
          if (buf[i] > maxv || (buf[i] == maxv && i > pos)) {
            maxv = buf[i];
            pos = i;
          }
        }
        
        // 正確的 θ 映射：0..255（最新→255，最舊→0）
        int idx = 255 - pos;

        // (3) 延遲線更新
        for (int i = N_DELAY - 1; i > 0; --i)
          delay_line[i] = delay_line[i - 1];
        delay_line[0] = static_cast<ux_theta_t>(idx & 0xFF);
