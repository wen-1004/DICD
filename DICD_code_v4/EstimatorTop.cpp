...
struct EstimatorTop : sc_core::sc_module {
  ...
  sc_core::sc_out<theta_t>  theta_out {"theta_out"};      // θ index (0..255)
  sc_core::sc_out<eps_t> eps_out   {"eps_out"};        // ε

  // ---------- Internal signals ----------
  // delay_n outputs
  sc_core::sc_signal<r_t> r_d1_real       {"r_d1_real"};       // r(k) after 1-cycle
  sc_core::sc_signal<r_t> r_d1_imag       {"r_d1_imag"};
  sc_core::sc_signal<r_t> r_dN_out_real   {"r_dN_out_real"};   // r(k+N)
  ...
  SC_CTOR(EstimatorTop) {
    ...
    U_MAG       = new mag       ("mag");
    U_ANGLE     = new angle     ("angle", /*delay_cycles=*/7);
    ...
    U_PHI_SUM->rho_in(rho_in);                     // if used internally
